...
MODELS_ROOT="${MODELS_DIR}/models_3_states"
RESULTS_ROOT="${RESULTS_DIR}/results_3_states"
echo "============================================"
echo " Ejecutando HVite y HResults para 3 estados"
echo "============================================"
for OUTER in {01..10}; do
  ...
  echo ">>> HVite: Fold=${OUTER} (3 estados)"
  HVite -H "$MACROS_FILE" \
        -H "$HMMDEFS_FILE" \
        -S "$TEST_SCP" \
        -i "$RECOUT_FILE" \
        "$DICT_FILE" \
        "$CLASSES_FILE"
  echo ">>> HResults: Fold=${OUTER} (3 estados)"
